...
interface LLMIntent {
  ...
  searchTerms: string[];
  ticketId?: string; // If a specific ID was mentioned
...
- ticketId: Only if they mentioned a specific ticket ID like "TEAM-1234"
...
Examples:
- "the upsells milestone" → {"entityType":"milestone","searchTerms":["upsells"],"ticketId":null,"projectHint":null,"milestoneHint":"upsells","assignee":null,"status":null,"cycle":null,"referencesFocus":false,"action":null}
- "TEAM-200" → {"entityType":"ticket","searchTerms":[],"ticketId":"TEAM-200","projectHint":null,"milestoneHint":null,"assignee":null,"status":null,"cycle":null,"referencesFocus":false,"action":null}
- "ok" (with focus) → {"entityType":"unknown","searchTerms":[],"ticketId":null,"projectHint":null,"milestoneHint":null,"assignee":null,"status":null,"cycle":null,"referencesFocus":true,"action":"proceed"}
- "my todo tickets in current cycle" → {"entityType":"ticket","searchTerms":[],"ticketId":null,"projectHint":null,"milestoneHint":null,"assignee":"me","status":"todo","cycle":"current","referencesFocus":false,"action":null}`;
...
${IntentResolver.INTENT_SYSTEM_PROMPT}`;
  ...
  async #searchEntities(
    ...
    if (!searchQuery && !intent.ticketId && !hasFilters) {
      return {
        status: "not_found",
        rawText: text,
        method: "llm_search",
    ...
    if (intent.ticketId) {
      return await this.#resolveTicketId(text, intent.ticketId);
    }
 ...
 */
export function createIntentResolver(deps: {
  issueTracker: IssueTracker;
  opencode: OpenCodeClient;
}): IntentResolver {
