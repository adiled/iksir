 ...
 * Sends results back to orchestrator session.
 *
...
export class ToolExecutor {
  ...
  /**
   * Set dispatcher (called after dispatcher is created to avoid circular dep)
   */
  ...
  async #handleEvent(event: MunToolCall): Promise<void> {
    ...
    if (ToolExecutor.GIT_TOOLS.has(event.tool) && this.#sessionManager.isGitFenced()) {
      ...
      const targetId = ("orchestratorId" in event && event.orchestratorId)
        ? event.orchestratorId as string
        : null;
      if (targetId) {
        await this.#sessionManager.sendToOrchestratorById(targetId, `## Tool Result: ${event.tool}\n\n${msg}`);
      } else {
        await this.#sessionManager.sendToOrchestrator(`## Tool Result: ${event.tool}\n\n${msg}`);
      }
      return;
    }
    ...
    try {
      switch (event.tool) {
        case "mun_read_wasfa":
          result = await this.#aalajaQiraatWasfa(event);
          break;
      ...
      // Send result back to the originating orchestrator (not necessarily the active one)
      const targetId = ("orchestratorId" in event && event.orchestratorId)
        ? event.orchestratorId as string
        : null;
      if (targetId) {
      ...
      const targetId = ("orchestratorId" in event && event.orchestratorId)
        ...
        : null;
      if (targetId) {
        await this.#sessionManager.sendToOrchestratorById(targetId, `## Tool Error: ${event.tool}\n\n${errorMsg}`);
      } else {
        await this.#sessionManager.sendToOrchestrator(`## Tool Error: ${event.tool}\n\n${errorMsg}`);
      }
    }
  }
   ...
   */
  async #aalajaQiraatWasfa(call: NidaQiraatWasfa): Promise<string> {
    ...
    if (parsed.type === "ticket") {
      ...
      const implStatusDb = getImplStatus(issue.identifier);
      parts.push(`## Context`);
      if (implStatusDb) {
...
Ready for pm_create_risala.`;
  ...
  async #aalajaKhalqRisala(call: NidaKhalqRisala): Promise<string> {
    ...
    const activeOrch = this.#sessionManager.getActiveOrchestrator();
    upsertImplStatus({
      wasfaId: call.wasfaId,
      orchestratorId: activeOrch?.identifier ?? "unknown",
      status: "complete",
    ...
    const activeOrchestrator = this.#sessionManager.getActiveOrchestrator();
    if (activeOrchestrator) {
      await this.#sessionManager.registerPR(activeOrchestrator.identifier, {
        wasfaId: call.wasfaId,
      ...
      await logger.warn("tool-executor", `PR #${result.number} created but no active orchestrator to track it`);
    }
...
${comparison.behind > 0 ? "⚠️ Branch is behind - consider rebasing before PR." : "✓ Branch is up to date with main."}`;
  ...
  async #handleNotify(call: MunNotifyCall): Promise<string> {
    ...
    if (!classification.worthy) {
      // Rejected - return guidance to orchestrator
...
Reason: ${classification.reason}`;
    ...
    // Send via messenger (routed to orchestrator's channel if available)
    if (this.#messenger.isEnabled()) {
      try {
        await this.#messenger.send(
          { orchestrator: call.orchestratorId },
          call.message,
        );
        sent.push("messenger");
      } catch (err) {
...
Message preview: ${call.message.slice(0, 100)}${call.message.length > 100 ? "..." : ""}`;
  ...
  async #handleReply(call: MunReplyCall): Promise<string> {
    ...
    try {
      ...
      await logger.info("tool-executor", "Reply sent to operator", {
        orchestratorId: call.orchestratorId,
        messagePreview: call.message.slice(0, 100),
      });
      return `Reply sent to operator (${call.orchestratorId}).`;
    } catch (err) {
  ...
  /**
   * Handle pm_yield - orchestrator voluntarily yields control
   */
  async #handleYield(call: MunYieldCall): Promise<string> {
    ...
    const yielderId = call.orchestratorId;
    const activeEpicId = this.#dispatcher.getActiveIdentifier();

    // Verify caller is the active orchestrator
    if (yielderId !== activeEpicId) {
      await logger.warn("tool-executor", `Non-active orchestrator ${yielderId} tried to yield (active: ${activeEpicId})`);
      return `Cannot yield: you (${yielderId}) are not the active orchestrator. Active: ${activeEpicId ?? "none"}.`;
    }

    // Update session status
    const newStatus = call.reason === "masdud" ? "masdud" : "muntazir";
    await this.#sessionManager.updateOrchestratorStatus(yielderId, newStatus, call.details);

    await logger.info("tool-executor", `Orchestrator ${yielderId} yielded: ${call.reason}`, {
    ...
    if (demands.length > 0) {
      ...
      removePendingDemand(demand.orchestrator_id);
      await logger.info("tool-executor", `Processing pending demand from ${demand.orchestrator_id}`, {
        reason: demand.reason,
      });

      const switchResult = await this.#dispatcher.handleCallback("cli", `switch:${demand.orchestrator_id}`);
      if (switchResult.handled) {
        return `Yielded control. Switching to ${demand.orchestrator_id} (pending demand: ${demand.reason}).`;
      }
    }
    ...
    if (call.suggestNext) {
      ...
      const suggested = orchestrators.find((o) => o.identifier === call.suggestNext);
      if (suggested && suggested.status === "sakin") {
        await this.#dispatcher.handleCallback("cli", `switch:${call.suggestNext}`);
    ...
    const orchestrators = this.#sessionManager.getOrchestratorSessions();
    const idleSessions = orchestrators.filter(
    ...
    this.#dispatcher.setActiveSession(null);
  ...
  /**
   * Handle pm_demand_control - orchestrator demands control back
   */
  async #handleDemandControl(call: MunDemandControlCall): Promise<string> {
    ...
    const demanderId = call.orchestratorId;
    const demander = this.#sessionManager.getOrchestrator(demanderId);

    if (!demander) {
      return `Cannot demand control: unknown orchestrator ${demanderId}.`;
    }

    const activeEpicId = this.#dispatcher.getActiveIdentifier();

    await logger.info("tool-executor", `Orchestrator ${demanderId} demands control`, {
      reason: call.reason,
      priority: call.priority,
    ...
    if (!activeEpicId) {
      const result = await this.#dispatcher.handleCallback("cli", `switch:${demanderId}`);
      if (result.handled) {
    ...
    if (activeEpicId === demanderId) {
      return `You (${demanderId}) are already the active orchestrator.`;
    }

    // Case 3: Current active is blocked/waiting — graceful snatch
    const currentActive = this.#sessionManager.getOrchestrator(activeEpicId);
...
Try: git push -u origin ${branchName}`;
    ...
    const session = this.#sessionManager.getOrchestrator(call.identifier);
    if (session) {
