...
export class QuestionHandler {
  ...
  // Questions awaiting custom text input (keyed by orchestrator identifier)
  // When the operator clicks "Type answer...", we store the question ID here
  // and the next text message in that orchestrator's channel becomes the custom answer
  ...
  async handleQuestionAsked(event: QuestionAskedEvent): Promise<void> {
    ...
    // Find which orchestrator this session belongs to
    const orchestrator = this.#sessionManager.getOrchestratorSessions().find(
      (o) => o.id === sessionID
    );

    if (!orchestrator) {
      await logger.warn("question-handler", `Question from unknown session ${sessionID}`);
      // Reject - we don't know which orchestrator this is
    ...
    if (classification.classification === "CRY_BABY") {
      ...
      await this.#forwardToDaemon(sessionID, id, questions, orchestrator.identifier);
    }
  }
...
Auto-selected: ${answers.map((a) => a.selected.join(", ")).join("; ")}`;
  ...
  async #forwardToDaemon(
    ...
    // Build question text
    const messageText = this.#formatQuestionMessage(primaryQuestion!, orchestratorId);

    // Send via messenger
    await this.#messenger.sendFormatted({ orchestrator: orchestratorId }, messageText);
   ...
   */
  #formatQuestionMessage(question: QuestionInfo, orchestratorId: string): string {
    let msg = `**${question.header}** (${orchestratorId})\n\n`;
   ...
   * The next text message in the orchestrator's channel will be used as the answer.
   */
  async markAwaitingCustomInput(orchestratorId: string, questionId: string): Promise<void> {
    this.#awaitingCustomInput.set(orchestratorId, questionId);
    // Persist state so we remember awaiting input after restart
    await this.saveState();
  }

  /**
   * Check if an orchestrator is awaiting custom input.
   */
  isAwaitingCustomInput(orchestratorId: string): boolean {
    return this.#awaitingCustomInput.has(orchestratorId);
  }
   ...
   */
  async handlePotentialCustomAnswer(orchestratorId: string, text: string): Promise<boolean> {
    const questionId = this.#awaitingCustomInput.get(orchestratorId);
    if (!questionId) {
      return false;
    }

    // Clear the awaiting state
    this.#awaitingCustomInput.delete(orchestratorId);
  ...
  async loadState(): Promise<void> {
    try {
      ...
      for (const dbQ of dbQuestions) {
        ...
        // Resolve orchestrator identifier from session ID
        const orchestrator = this.#sessionManager.getOrchestratorSessions().find(
        ...
        const pendingQuestion: PendingQuestion = {
          ...
          orchestratorId: orchestrator?.identifier ?? dbQ.session_id,
          questions: [{
 ...
 */
export function createQuestionHandler(deps: QuestionHandlerDeps): QuestionHandler {
