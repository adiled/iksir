 ...
 * Dispatcher
 *
 * Routing layer between inbound interfaces (Telegram, CLI, PR comments)
 * and orchestrator sessions.
 *
 * Responsibilities:
 * - Parse intent from raw input (deterministic + LLM hybrid)
 * - Classify as query vs operation
 * - Route to correct orchestrator session
 * - Manage active session (only one can do git ops)
 * - Queue operations for idle sessions
 * - Handle disambiguation when multiple matches found
 * - Suggest parent epics for child tickets
 */
...
import type { MunadiConfig, OrchestratorSession, MessengerOutbound } from "../types.ts";
...
/**
 * Parameters for creating/activating an orchestrator.
 * Used by the common #createAndActivateOrchestrator method.
 */
interface CreateOrchestratorParams {
  identifier: string;
  title: string;
  type: "epic" | "chore" | "sandbox";
  /** Init message to send to new orchestrators */
  initMessage: string;
  /** Optional ticket URL for ticket-based orchestrators */
  url?: string;
}
...
export class Dispatcher {
   ...
   * Always uses intent resolver — never short-circuits to active orchestrator.
   * Dispatch is for ticket lookups, spawning orchestrators, and commands.
   */
  ...
  async #handleInbound(msg: InboundMessage): Promise<DispatchResult> {
    ...
    // Default routing: if active orchestrator exists and message doesn't
    // explicitly reference a different ticket, route directly to active
    if (this.#activeIdentifier && !basicIntent.identifier) {
      const session = this.#sessionManager.getOrchestratorSessions().find(
        (s) => s.identifier === this.#activeIdentifier
      );
      if (session) {
        await logger.info("dispatcher", `Routing to active orchestrator: ${this.#activeIdentifier}`);
  ...
  /**
   * Clear recent messages after starting/resuming an orchestrator
   */
  ...
  async #handleResolvedIntent(
    ...
    // Check if orchestrator already exists for this ticket
    const existingSession = this.#sessionManager.getOrchestratorSessions().find(
    ...
    // Check if orchestrator exists for the parent
    if (resolved.parentEpic) {
      const parentId = resolved.parentEpic.identifier;
      const parentSession = this.#sessionManager.getOrchestratorSessions().find(
        (s) => s.identifier === parentId
      );

      if (parentSession) {
        // Parent has orchestrator - route there instead
        await logger.info("dispatcher", `Routing to parent orchestrator: ${parentId}`);
        return this.#routeToSession(parentSession, msg);
      }

      // No orchestrator for either - suggest working on parent if it's an epic
    ...
    // Need to start a new orchestrator
    ...
    if (basicIntent.type === "query" && !isProceeding) {
      return {
        handled: true,
        response: `Found ${entity.type}: ${escapeMarkdown(entity.title)}\n\nNo orchestrator running for this. Say "work on ${epicId}" to start one.`,
      };
    }

    // Start orchestrator for this entity
    return this.#startOrchestratorForEntity(msg, entity);
  }
  ...
  async #handleDisambiguationSelection(
    ...
    // Start orchestrator for selected entity
    return this.#startOrchestratorForEntity(
   ...
   * Only called when:
   * - Ticket has a parent
   * - No orchestrator exists for either ticket or parent
...
Work on the parent instead?`;
  ...
  async #handleParentSelection(source: InboundSource, useParent: boolean): Promise<DispatchResult> {
    ...
    return this.#startOrchestratorForEntity({ source, text: "" }, entity);
  }

  // ===========================================================================
  // Orchestrator Creation (Common Path)
  // ===========================================================================

  /**
   * Common orchestrator creation and activation.
   * Handles:
   ...
   *
   * Both ticket-based and sandbox orchestrators use this path.
   */
  async #createAndActivateOrchestrator(
    params: CreateOrchestratorParams
  ): Promise<DispatchResult> {
    const { identifier, title, type, initMessage, url } = params;

    await logger.info("dispatcher", `Creating/activating orchestrator: ${identifier}`, { type, title });

    // Step 1: Get or create session
    const result = await this.#sessionManager.getOrCreateOrchestrator(identifier, title, type);

    if (!result) {
      return {
        handled: true,
        error: "Failed to create orchestrator session.",
      };
    }
    ...
    const switchResult = await this.#executeOrchestratorSwitch(
      identifier,
      session,
      previousActive,
      isNew
    ...
    // Step 4: Send appropriate message to orchestrator
    if (isNew) {
      await this.#sessionManager.sendToOrchestrator(initMessage);
    } else if (wasResumed) {
      await this.#sessionManager.sendToOrchestrator(
    ...
    if (isNew) {
      response = `**New orchestrator started for ${identifier}**\n\n`;
  ...
  /**
   * Execute the formal switchover for an orchestrator.
   * Handles WIP commit, branch checkout, and state updates.
   * Used for both new and existing orchestrators.
   */
  async #executeOrchestratorSwitch(
    identifier: string,
    ...
    const previousSession = previousActive
      ? this.#sessionManager.getOrchestrator(previousActive)
      : null;
    ...
    try {
    // Step 1: Interrupt previous orchestrator if exists
    if (previousSession) {
    ...
    if (previousSession) {
      await this.#sessionManager.updateOrchestratorStatus(previousActive!, "sakin");
      await this.#notifyPreviousSession(previousSession, wipCommitted);
    }

    // Step 4: Checkout target branch (creates if doesn't exist for new orchestrators)
    const checkoutSuccess = await git.checkout(session.branch);
    if (!checkoutSuccess) {
      // Abort switch - restore previous as active
      if (previousSession) {
        await this.#sessionManager.updateOrchestratorStatus(previousActive!, "fail");
      }
      return {
        handled: true,
    ...
    // Step 5: Activate new orchestrator (DB first, then local state)
    try {
      await this.#sessionManager.updateOrchestratorStatus(identifier, "fail");
      this.#sessionManager.setActiveOrchestrator(identifier);
      ...
      if (previousSession) {
        await this.#sessionManager.updateOrchestratorStatus(previousActive!, "fail").catch(() => {});
        this.#sessionManager.setActiveOrchestrator(previousActive);
      }
  ...
  /**
   * Start orchestrator for a ticket entity (ticket/epic/project)
   */
  async #startOrchestratorForEntity(
    _msg: InboundMessage,
...
Use \`pm_read_ticket\` to fetch full details and begin planning.`;
    ...
    // Map entity type to orchestrator type
    const orchestratorType: "epic" | "chore" =
      (entity.type === "epic" || entity.type === "project" || entity.type === "milestone")
        ? "epic"
        : "chore";

    return this.#createAndActivateOrchestrator({
      identifier,
      title: entity.title,
  ...
  /**
   * Public entry point for activating an orchestrator from a ticket URL.
   * Routes through the full switch protocol (#createAndActivateOrchestrator)
...
When you want to formalize this work into tickets, let the operator know.`;
  ...
  async #handleBasicMessage(msg: InboundMessage, intent: Intent): Promise<DispatchResult> {
    ...
    if (!session) {
      return {
        handled: true,
        response: `No orchestrator for ${targetId}. Send a ticket URL to start one.`,
      };
    }
  ...
  /**
   * Route message to orchestrator session
   */
  async #routeToSession(
    ...
    const success = await this.#sessionManager.sendToOrchestratorById(
      session.identifier,
      `${prefix}${msg.text}`
    );
  ...
  /**
   * Restore active orchestrator on daemon startup.
   * Uses the centralized #executeOrchestratorSwitch to ensure branch checkout,
   * notification, and all other switchover logic happens.
   */
  async restoreActiveOnStartup(): Promise<void> {
    const activeId = this.#sessionManager.getActiveOrchestratorId();
    if (!activeId) {
      await logger.info("dispatcher", "No active orchestrator on startup");
      return;
    }

    const session = this.#sessionManager.getOrchestrator(activeId);
    if (!session) {
      await logger.warn("dispatcher", `Active orchestrator ${activeId} not found in session manager`);
      return;
    }

    await logger.info("dispatcher", `Restoring active orchestrator on startup: ${activeId}`);

    // Use centralized switch logic - handles branch checkout, notification, queue, etc.
    await this.#executeOrchestratorSwitch(activeId, session, null, false);
  }

  /**
   * Manual switch command handler. Validates target exists, delegates to
   * #executeOrchestratorSwitch for the full switch protocol (interrupt,
  ...
  async #switchActiveSession(epicId: string, _source: InboundSource): Promise<DispatchResult> {
    const targetSession = this.#sessionManager.getOrchestrator(epicId);

    if (!targetSession) {
      return { handled: true, response: `No orchestrator for ${epicId}. Start one first.` };
    }
    ...
    // Return simple confirmation (dispatch notification sent by #executeOrchestratorSwitch)
    return { handled: true, response: `Active: **${epicId}**` };
  }

  /**
   * Send INTERRUPT to previous orchestrator and all its implementors
   */
  async #interruptSession(session: OrchestratorSession, newActiveId: string): Promise<void> {
    const interruptMsg = `🛑 INTERRUPT: Control is being transferred to ${newActiveId}.

STOP all operations immediately.
Do NOT make any more git operations.
Do NOT invoke any more implementors.

You will be notified when you are IDLE.`;

    // Interrupt orchestrator
    await this.#sessionManager.sendToOrchestratorById(session.identifier, interruptMsg);
   ...
   */
  async #notifyPreviousSession(session: OrchestratorSession, wipCommitted: boolean): Promise<void> {
...
Use \`pm_demand_control\` when you have actionable work.`;
    ...
    await this.#sessionManager.sendToOrchestratorById(session.identifier, msg);
  }

  /**
   * Notify new session that it's now ACTIVE
   */
  async #notifyNewActiveSession(session: OrchestratorSession, previousId: string | null): Promise<void> {
...
You may now perform git operations.`;
  ...
  #getStatus(): DispatchResult {
    ...
    const orchestrators = this.#sessionManager.getOrchestratorSessions();

    let response = "**Status**\n\n";

    // Active session (prominent)
    if (active) {
      const activeSession = orchestrators.find((o) => o.identifier === active);
    ...
    // Summary
    response += `Orchestrators: ${orchestrators.length}\n`;
    response += `Queue: ${queueLen} operation(s)\n`;

    // List other orchestrators (if any)
    const others = orchestrators.filter((o) => o.identifier !== active);
    if (others.length > 0) {
      response += `\n**Other orchestrators (idle):**\n`;
  ...
  #getSessionsStatus(): DispatchResult {
    ...
    const idleSessions = orchestrators.filter((o) => o.identifier !== this.#activeIdentifier);
  ...
  #showSwitchPicker(): DispatchResult {
    ...
    if (orchestrators.length === 0) {
      return { handled: true, response: "No orchestrators to switch to." };
    }
    ...
    if (idleSessions.length === 0) {
      ...
      return {
        handled: true,
        response: `Only one orchestrator exists: ${active} (already active)`,
      };
    }
    ...
    if (this.#activeIdentifier) {
      const activeSession = orchestrators.find((o) => o.identifier === this.#activeIdentifier);
