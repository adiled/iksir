 ...
 * These feed information to the owning orchestrator session.
 */
interface KeepAliveCallbacks {
  /**
   * PR was merged - orchestrator can now disclose next dependent slice
   */
  onPRMerged: (
  ...
  /**
   * Request maintenance mode - daemon should ensure no orchestrator is active
...
export class KeepAliveLoop {
   ...
   * Polls all tracked PRs across all orchestrator sessions.
   */
   ...
   */
  async #pollPR(session: OrchestratorSession, trackedPR: TrackedPR): Promise<void> {
  ...
  async #runMaintenance(): Promise<void> {
    ...
    // Request maintenance mode (no active orchestrator)
    const granted = await this.#callbacks.requestMaintenanceMode();
    if (!granted) {
      await logger.warn("keepalive", "Maintenance mode denied - orchestrator active");
    ...
    try {
      ...
      const sessions = this.#sessionManager.getOrchestratorSessions();
   ...
   */
  async #maintainBranch(session: OrchestratorSession): Promise<MaintenanceResult> {
