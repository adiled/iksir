...
Deno.test("generateBranchName: chore uses MUNADI_GIT_USER prefix", () => {
  const prev = Deno.env.get("MUNADI_GIT_USER");
  Deno.env.set("MUNADI_GIT_USER", "testuser");
  try {
    assertEquals(
    ...
    if (prev) Deno.env.set("MUNADI_GIT_USER", prev);
    else Deno.env.delete("MUNADI_GIT_USER");
  }
});

Deno.test("generateBranchName: chore defaults to dev/ prefix", () => {
  const prev = Deno.env.get("MUNADI_GIT_USER");
  Deno.env.delete("MUNADI_GIT_USER");
  try {
    assertEquals(
      generateBranchName("TEAM-300", "chore"),
      "dev/TEAM-300",
