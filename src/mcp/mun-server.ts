 ...
 * Munadi MCP Server
 *
 * Provides tools to the Orchestrator LLM:
 ...
 *
 * Communicates with Munadi daemon via SQLite IPC (events table).
 */
...
class MunadiToolRegistry implements ToolRegistry {
...
export class MunadiMunMcpServer {
  #registry: MunadiToolRegistry;

  constructor() {
    // Create the registry with our IPC forwarder
    this.#registry = new MunadiToolRegistry((call) => this.#forwardToDaemon(call));
  ...
  #registerCoreTools(): void {
    ...
    this.#registry.register(
      {
        ...
        description:
          "Set blocking relations between tickets. Primary mechanism for orchestrating execution order.",
        inputSchema: {
          type: "object",
          properties: {
            huwiyyatMurshid: {
    ...
    this.#registry.register(
      {
        name: "mun_read_wasfa",
        description: `Read any issue tracker URL (Linear, Jira, GitHub) and get enriched information with Munadi context.
...
- Munadi context (connected implementor sessions, implementation status, PR info)
...
The diary is a shared knowledge pool across all orchestrators. Use it to:
...
- Get context when taking over from another orchestrator`,
        inputSchema: {
          type: "object",
          properties: {
            huwiyyatMurshid: {
            ...
            filterOrchestrator: {
              type: "string",
              description: "Filter by a specific orchestrator ID (omit for collective pool)",
            },
            type: {
              type: "string",
...
If no other orchestrator is active, you'll be granted control immediately.
If another orchestrator is working, Operator will be asked to approve the switch.`,
        inputSchema: {
          type: "object",
          properties: {
            huwiyyatMurshid: {
    ...
    this.#registry.register(
      {
        ...
        description: `Create the branch for a new orchestrator. Called once when starting work.
...
You should only call this once per orchestrator, at the start.`,
        inputSchema: {
          type: "object",
          properties: {
            huwiyyatMurshid: {
            ...
            type: {
              ...
              description: "Type of orchestrator: 'epic' for multi-ticket work, 'chore' for standalone tasks, 'sandbox' for freeform work",
            },
            slug: {
              type: "string",
...
4. Enrich with Munadi context (sessions, implementation status, PRs)
...
This decision is now part of the persistent record.`;
  ...
  #handleReadDiary(args: Record<string, unknown>): string {
    ...
    if (decisions.length === 0) {
      const filters = [
        call.filterOrchestrator && `orchestrator=${call.filterOrchestrator}`,
...
What happens next:
- If queue has pending work → another orchestrator becomes active
- If other orchestrators have work → Operator can approve switch
...
Daemon will:
1. If no active orchestrator → grant control immediately
...
You will be notified when control is granted.`;
  ...
  async #handleCreateBranch(args: Record<string, unknown>): Promise<string> {
    ...
    const branchName = generateBranchName(call.identifier, orchestratorType, call.slug);
